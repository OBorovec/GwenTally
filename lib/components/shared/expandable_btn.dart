...
class ExpandableBtn extends StatefulWidget {
  ...
  @override
  State<ExpandableBtn> createState() => _ExpandableBtnState();
}

class _ExpandableBtnState extends State<ExpandableBtn>
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      height: 2 * ExpandableBtn.maxDistance,
      width: 2 * ExpandableBtn.maxDistance,
      child: Stack(
  ...
  List<Widget> _buildExpandingActionButtons(Alignment alignment) {
    ...
    if (alignment == Alignment.center) {
      ...
      offsetRoot = const Offset(
        ExpandableBtn.maxDistance - 33,
        ExpandableBtn.maxDistance - 24,
      );
    } else if (alignment == Alignment.centerRight) {
      step = 180 / (count - 1);
      initAngle = 270;
      offsetRoot = const Offset(0, ExpandableBtn.maxDistance - 24);
    } else if (alignment == Alignment.centerLeft) {
      step = 180 / (count - 1);
      initAngle = 90;
      offsetRoot = const Offset(
        2 * ExpandableBtn.maxDistance - 66,
        ExpandableBtn.maxDistance - 24,
      );
    } else if (alignment == Alignment.bottomCenter) {
      step = 180 / (count - 1);
      initAngle = 0;
      offsetRoot = const Offset(ExpandableBtn.maxDistance - 24, 0);
    } else {
      step = 90.0 / (count - 1);
      initAngle = 0;
